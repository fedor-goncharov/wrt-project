...
double plane_integral3d(double (*function_ptr)(double, double, double), 
  ...
  for (i_v1 = -(nsize / 2); i_v1 < (nsize/2); ++i_v1) {		// integration along v1
    for (i_v2 = -(nsize / 2); i_v2 < (nsize/2); ++i_v2) {	// integration along v2
        vect3d point;
	...
	double weight = (*weight_function_ptr)(point.x, point.y, point.z, phi, theta);
	
	integral += value * weight * delta * delta; 
    }
  }  
  return integral;
}
