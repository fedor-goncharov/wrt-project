...
def generate_noise_xray_transmission(input_filename, output_filename, avg_intensity=1e5, T=1.0, sc_intensity=1e1):
...
def generate_noise_emission(input_filename, output_filename, N_max, N_sc):
    """
    function (1) reads weighted ray/Radon transforms - (2) finds maximal value P_wf and normalizes data P_wf / max(P_wf) -
    (2) generates Poisson rdvs Po(N_max * P_wf / max(P_wf) + r), where N_max is given in the arguments
    ...
    Example : 
        generate_noise_emission('ray_transforms.bin', 'noise_ray_transforms.bin', 1e2, 3)
    """
