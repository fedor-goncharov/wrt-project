...
"""
 ...
 Function takes on input a three-dimensional image and returns its spectral
 Laplace transform in 3D.
 ...
 RETURN VALUE :
     Three-dimensional matrix of size ngrid x ngrid x ngrid
...
def spectral_laplace3d(u, ngrid, period=2.0):
  ...
  u1, u2, u3 = np.meshgrid(freq_factor, freq_factor, freq_factor)
  U = ((2*np.pi / period)**2)*((u1*u1 + u2*u2 + u3*u3))
