...
double ray_integral3d(double (*function_ptr)(double, double, double),
		      double (*weight_function_ptr)(double, double, double, double),
  ...
  for (i_v = -(nsize / 2); i_v < (nsize/2) + 1; ++i_v) {
     vect3d point;
	   ...
	   double weight_term = (*weight_function_ptr)(point.x, point.y, point.z, phi);

	   integral += weight_term * value * delta; //1D integration along direction
  }
  return integral;
}
